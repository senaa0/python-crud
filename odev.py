...
def asd (rows):
    ttk.Treeview.delete(*ttk.Treeview.get_children())
    for i in rows:
        ttk.Treeview.insert("", "end", values=i) 
...
def GetValue(event):
    ...
    e6.insert(0, select['telefon'])
    e7.insert(0, select['sehir'])
...
def Add():
    ...
    try:
        ...
        cursor.execute(query,(id, cariadi, vno, vergidairesi, adres, telefon, sehir))
        clear()
...
def update():
    ...
    try:
        ...
        e7.delete(0, END)
        e1.focus_set()
        cursor.execute(query,(cariadi, vno, vergidairesi, adres, telefon, sehir, id))
        clear()
...
Button(root, text="Ekle", command=Add, height=3,
       width=13, bg="green",).place(x=30, y=130)
Button(root, text="Güncelle", command=update, height=3,
       width=13, bg="blue",).place(x=140, y=130)
Button(root, text="Sil", command=delete, height=3,
       width=13, bg="red",).place(x=250, y=130)
